...
def eval_train_2dgmm(net, eval_loader, prototype_manager, device, tau=0.1, use_oracle=False, true_labels_tensor=None, predicted_noise_rate=None):
    ...
    # 合理性检查2：干净分量权重>95%且L2范数差距不大，交换分量
...
def main():
    ...
    if args.load_state_dict is not None:
        ...
        if 'predicted_noise_rate' in checkpoint:
            ...
            # 旧版本checkpoint，需要重新预测噪声率（不能使用真实噪声率）
            print('Old checkpoint format detected, attempting to recover noise rate history...')
            
            # 尝试从fix.txt日志文件恢复历史噪声率
            history_recovered = False
            fix_log_path = os.path.join(p['scanmix_dir'], 'fix.txt')
            if os.path.exists(fix_log_path):
                try:
                    ...
                    # 查找所有噪声率记录（格式: "Epoch:X [1D-GMM] Current: 0.xxxx, MAX: 0.xxxx, MIN: 0.xxxx"）
                    noise_rates = []
                    for line in lines:
                        if '[1D-GMM]' in line and 'Current:' in line:
                            try:
                                ...
                                max_val = float(parts[1].split('MAX:')[1].strip())
                                min_val = float(parts[2].split('MIN:')[1].strip())
                                noise_rates.append((current, max_val, min_val))
                            except:
                                continue
                    
                    if noise_rates:
                        # 使用最后一次记录的值
                        last_current, last_max, last_min = noise_rates[-1]
                        predicted_noise_rate = last_current
                        noise_rate_history_max = last_max
                        noise_rate_history_min = last_min
    ...
    # 只在从头训练时初始化噪声率追踪变量（checkpoint恢复时已经设置）
    if start_epoch == 0:
        predicted_noise_rate = None  # 保存预测的噪声率用于后续2D-GMM筛选限制
        initial_noise_rate = None  # 第一次预测的噪声率，用于判断高/低噪声场景
        noise_rate_history_max = None  # 历史最大噪声率
        noise_rate_history_min = None  # 历史最小噪声率
        is_high_noise_scenario = None  # True: 高噪声场景(>=50%)，取最大值; False: 低噪声场景(<50%)，取最小值
    # else: 这些变量已经在checkpoint加载时恢复
    ...
    for epoch in range(start_epoch, p['num_epochs']+1):   
        ...
        if epoch<p['warmup']:       
            ...
            if epoch == p['warmup']-1:
                ...
                log_info(f'[NOISE RATE STRATEGY] Initial noise rate: {initial_noise_rate:.1%}')
                if is_high_noise_scenario:
                    log_info(f'  -> HIGH NOISE scenario (>=50%): Will use MAXIMUM noise rate in future')
                else:
                    log_info(f'  -> LOW NOISE scenario (<50%): Will use MINIMUM noise rate in future')
                fix_log.write('Epoch:%d PREDICTED NOISE RATE: %.4f (Net1: %.4f, Net2: %.4f)\n'%(epoch, predicted_noise_rate, noise1, noise2))
                fix_log.write('Initial noise rate: %.4f, Strategy: %s\n'%(initial_noise_rate, 'HIGH_MAX' if is_high_noise_scenario else 'LOW_MIN'))
            ...
            # ========== 每5个epoch重新计算噪声率（使用1D-GMM）并与历史对比选取 ==========
            if (epoch - p['warmup']) % 5 == 0:
                print('\n[Noise Rate Re-estimation with 1D-GMM]')
                print(f'Re-calculating noise rate at epoch {epoch} using 1D-GMM...')
                ...
                current_noise_rate = (temp_noise1 + temp_noise2) / 2
                
                # 显示当前计算出的噪声率
                log_info(f'[1D-GMM] Current calculated noise rate: {current_noise_rate:.4f} (Net1: {temp_noise1:.4f}, Net2: {temp_noise2:.4f})')
                
                # 更新历史最大/最小值
                noise_rate_history_max = max(noise_rate_history_max, current_noise_rate)
                noise_rate_history_min = min(noise_rate_history_min, current_noise_rate)
                
                # 根据场景选择使用的噪声率（与历史对比选取）
                if is_high_noise_scenario:
                    # 高噪声场景：选择历史最大值（防止噪声率下降导致选入过多噪声样本）
                    predicted_noise_rate = noise_rate_history_max
                    strategy_msg = f'HIGH noise strategy: comparing with history and using MAX'
                else:
                    # 低噪声场景：选择历史最小值（防止噪声率上升导致丢弃过多干净样本）
                    predicted_noise_rate = noise_rate_history_min
                    strategy_msg = f'LOW noise strategy: comparing with history and using MIN'
                
                log_info(f'[1D-GMM] History comparison: MAX={noise_rate_history_max:.4f}, MIN={noise_rate_history_min:.4f}')
                log_info(f'[1D-GMM] {strategy_msg} -> Final predicted_noise_rate={predicted_noise_rate:.4f}')
                fix_log.write('Epoch:%d [1D-GMM] Current: %.4f, MAX: %.4f, MIN: %.4f, Used: %.4f (%s)\n' % (
                    epoch, current_noise_rate, noise_rate_history_max, noise_rate_history_min, 
                    predicted_noise_rate, 'HIGH_MAX' if is_high_noise_scenario else 'LOW_MIN'))
                fix_log.flush()
