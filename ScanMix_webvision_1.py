...
def main():
    ...
    print('| Building net')
    net1 = create_model(device)
    net2 = create_model(device)

    net1_clone = create_model(device)
    net2_clone = create_model(device)
    ...
    for epoch in range(start_epoch, p['num_epochs']+1):   
        ...
        q = queue.Queue()
        scanmix_big_test(epoch, net1, net2_clone, test_loader, device, q)
        acc = q.get()
