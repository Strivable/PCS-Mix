...
meta_info['pred'] = None
meta_info['mode'] = 'all'  # Changed from 'pretext' to 'all' for proper dataset initialization
...
def main():
    ...
    # Create separate meta_info for validation with mode='test'
    val_meta_info = meta_info.copy()
    val_meta_info['mode'] = 'test'
    
    # Check if neighbor files exist (from pretext training)
    use_neighbors = os.path.exists(p['topk_neighbors_train_path']) and os.path.exists(p['topk_neighbors_val_path'])
    if not use_neighbors:
        print(colored('WARNING: Neighbor files not found. Please run pretext (MoCo/SimCLR) training first.', 'red'))
        print(colored(f"Expected files:\n  - {p['topk_neighbors_train_path']}\n  - {p['topk_neighbors_val_path']}", 'yellow'))
        raise FileNotFoundError('Neighbor files required for SCAN training. Run pretext training first.')
    
    train_dataset = get_train_dataset(p, train_transformations, 
                                        split='train', to_neighbors_dataset=use_neighbors, to_noisy_dataset=p['to_noisy_dataset'], meta_info=meta_info)
    val_dataset = get_val_dataset(p, val_transformations, to_neighbors_dataset=use_neighbors, meta_info=val_meta_info)
    ...
    if os.path.exists(p['scan_checkpoint']):
        ...
        checkpoint = torch.load(p['scan_checkpoint'], map_location='cpu', weights_only=False)
    ...
    model_checkpoint = torch.load(p['scan_model'], map_location='cpu', weights_only=False)
