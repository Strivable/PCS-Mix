...
def get_model(p, pretrain_path=None):
    # Get backbone
    if p['backbone'] == 'resnet18':
        if p['train_db_name'] in ['cifar-10', 'cifar-20', 'cifar-100', 'mini_imagenet_red', 'mini_imagenet32_red', 'neucls']:
        ...
        if 'webvision' in p['train_db_name']:
    ...
    if pretrain_path is not None and os.path.exists(pretrain_path):
        ...
        if p['setup'] in ['simclr', 'moco']:  # Direct loading for contrastive learning models
        ...
        elif p['setup'] == 'dividemix' or p['setup'] == 'scanmix': # Weights are supposed to be transfered from scan 
            # We only continue with the best head (pop all heads first, then copy back the best head)
            model_state = state['model']
            all_heads = [k for k in model_state.keys() if 'cluster_head' in k]
            best_head_weight = model_state['cluster_head.%d.weight' %(state['head'])]
...
def get_train_dataset(p, transform, to_augmented_dataset=False,
    ...
    elif p['train_db_name'] == 'neucls':
        from data.neucls import NEUCLSDataset, NoisyNEUCLS
        if to_noisy_dataset and meta_info.get('noise_file'):
            # 使用带噪声标签的数据集
            dataset = NoisyNEUCLS(
                root=p['data_path'],
                split='train',
                transform=None,  # transform会在wrapper中应用
                noise_file=meta_info['noise_file'],
                noise_mode=meta_info.get('noise_mode', 'sym'),
                noise_rate=meta_info.get('noise_rate', 0.0)
            )
        else:
            # 干净标签数据集
            dataset = NEUCLSDataset(root=p['data_path'], split='train', transform=transform)
...
def get_val_dataset(p, transform=None, to_neighbors_dataset=False, meta_info=None):
    ...
    elif p['val_db_name'] == 'neucls':
        from data.neucls import NEUCLSDataset
        dataset = NEUCLSDataset(root=p['data_path'], split='test', transform=transform)
...
def get_train_transformations(p):
    ...
    elif p['augmentation_strategy'] == 'simclr' or p['augmentation_strategy'] == 'moco':
        # Augmentation strategy from SimCLR/MoCo-v2 (they use similar augmentations)
        return transforms.Compose([
            transforms.RandomResizedCrop(**p['augmentation_kwargs']['random_resized_crop']),
            transforms.RandomHorizontalFlip(),
    ...
    elif p['augmentation_strategy'] == 'dividemix':
        trnfs = []
...
def get_val_transformations(p):
    ...
    if 'resize' in p['augmentation_kwargs'].keys():
        trnfs = [transforms.Resize(p['augmentation_kwargs']['resize'])]
    # 检查augmentation_strategy是否存在，如果不存在或不是dividemix_red_mini_imagenet，则添加CenterCrop
    if p.get('augmentation_strategy', '') != 'dividemix_red_mini_imagenet':
