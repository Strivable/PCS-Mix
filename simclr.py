...
def main():
    ...
    if os.path.exists(p['pretext_checkpoint']):
        ...
        # 检查checkpoint中的特征维度是否与当前配置一致
        try:
            # 尝试检查投影头最后一层的权重形状
            checkpoint_feature_dim = checkpoint['model']['contrastive_head.2.weight'].shape[0]
            current_feature_dim = p['model_kwargs']['features_dim']
            
            if checkpoint_feature_dim != current_feature_dim:
                print(colored(f'WARNING: Feature dimension mismatch!', 'red'))
                print(colored(f'  Checkpoint: {checkpoint_feature_dim}D, Current config: {current_feature_dim}D', 'red'))
                print(colored(f'  Skipping checkpoint and training from scratch...', 'yellow'))
                start_epoch = 0
                model = model.cuda()
            else:
                # 维度匹配，正常加载
                ...
                print(colored(f'Successfully loaded checkpoint (feature_dim={checkpoint_feature_dim})', 'green'))
        except Exception as e:
            print(colored(f'Error checking checkpoint compatibility: {e}', 'red'))
            print(colored('Training from scratch...', 'yellow'))
            start_epoch = 0
            model = model.cuda()
